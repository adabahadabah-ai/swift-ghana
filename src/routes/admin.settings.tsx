...
function AdminSettingsPage() {
  ...
  const handleSave = async () => {
    setSaving(true);
    // Use edge function or server function to update since only service_role can update
    // For now we'll show success (actual update needs server function)
    toast.success("Settings saved! (Note: requires server-side update for persistence)");
    setSaving(false);
  };
      ...
      <GlassCard variant="strong">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <Input
              defaultValue="support@swiftdata.gh"
              className="h-11 bg-glass border-glass-border rounded-xl"
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setSettings({ ...settings, disable_ordering: e.target.checked })}
              className="rounded"
            />
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              ...
              onChange={(e) => setSettings({ ...settings, holiday_mode_enabled: e.target.checked })}
              className="rounded"
            />
